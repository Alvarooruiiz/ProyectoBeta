...
public class SharedEditRegister extends AppCompatActivity implements OnImageClickListener {
    ...
    private void registerUser() {
        if (areFieldsFilled()) {
            ...
            if (newUri != null) {
                long userId = ContentUris.parseId(newUri); // Obtener el ID del usuario insertado

                // Insertar las imágenes asociadas al usuario en la tabla de imágenes
                for (Bitmap bitmap : imagesList) {
                    ...
                    // Crear ContentValues para la imagen
                ...
                // Tu código de notificación y redirección a la pantalla de inicio de sesión
            } else {
    ...
    public boolean editarUsuario(View view) {
        ...
        int rowsUpdated = getContentResolver().update(UsuarioProvider.CONTENT_URI_USUARIOS, values, selection, selectionArgs);
        ...
        if (resultCode == RESULT_OK) {
            if (requestCode == REQUEST_SELECT_IMAGE && data != null) {
                ...
                if (selectedImageUri != null) {
                    // Añadir la URI de la imagen seleccionada al ArrayList
    ...
    public void onImageClick(final int position) {
        new MaterialAlertDialogBuilder(this)
                .setTitle("Alerta")
                .setMessage("Estas seguro de que deseas eliminar la foto")
